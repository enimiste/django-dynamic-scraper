...
class DjangoSpider(DjangoBaseSpider):
        ...
        for rpt in self.scraper.requestpagetype_set.all():
            if rpt.form_data != '':
                try:
                    ...
                    raise CloseSpider(
                        "Incorrect form_data attribute ({pt}): not a valid JSON dict!".format(pt=rpt.page_type))
    ...
    def _set_start_urls(self, scrape_url):
        ...
        if self.scraper.pagination_type not in ['N', 'X']:
            ...
            if item:
                ...
                if self.scraper.get_detail_page_url_elems().count() == 0 or \
                    ...
                    for url_elem in url_elems:
                        ...
                        if rpt.request_type == 'R':
                            yield Request(url, callback=self.parse_item, method=rpt.method, dont_filter=rpt.dont_filter,
                                          **kwargs)
                        else:
                            yield FormRequest(url, callback=self.parse_item, method=rpt.method,
                                              formdata=self.dp_form_data[rpt.page_type], dont_filter=rpt.dont_filter,
                                              **kwargs)
            else:
